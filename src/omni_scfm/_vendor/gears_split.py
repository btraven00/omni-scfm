"""Vendored from GEARS (https://github.com/snap-stanford/GEARS), MIT License.

This module is a verbatim copy of the ``DataSplitter`` class and the three
perturbation-name parsing helpers it depends on, taken from
``gears/data_utils.py`` and ``gears/utils.py``.

Why vendored: GEARS computes the train/val/test split that the reference paper
(Ahlmann-Eltze et al., Nat Methods 2025) uses, but importing ``gears`` pulls in
torch + torch_geometric via ``gears/__init__.py`` and ``gears/utils.py``.
``DataSplitter`` itself needs only NumPy and these tiny string helpers, so we
copy them here to reproduce the exact split in a lightweight environment.

The split logic below is UNMODIFIED upstream code (only the torch-importing
helpers it does not use were dropped). Do not edit it; to update, re-copy from
the corresponding upstream commit.

Upstream copyright (c) the GEARS authors, Stanford SNAP. MIT License.
...
# --- from gears/data_utils.py (verbatim) -------------------------------------
class DataSplitter():
    """
